...
/* document.listForm.sch_type.value = "${sch_type}"; */
  $(document).ready(function() { 
	...
	<script type="text/javascript">
	function adminList() {
		   location.href = "admin_list.do?board_id=10";
		}
	</script>
<style>
.mouseOverHighlight {
...
<%-- 				<input type="hidden" id="admin_num" name="admin_num" value="${admin_num}">
					...
					<c:choose>
						...
						<c:otherwise>
							<c:forEach var="admin_list" items="${admin_list}"  varStatus="status">
									...
									<c:if test="${admin_list.admin_id ne 'bpfcadmin' && admin_list.admin_id ne 'interlink'}">
								<tr>
									<td align="center">
									...
									<td align="center">
									<a id="adminid" class="mouseOverHighlight" onclick="location.href='admin_view.do?board_id=10&admin_id=${admin_list.admin_id}'">&nbsp;${admin_list.admin_id}</a>
									...
									<td align="center">
									<input class="btn btn-primary" type="button" name="preview" id="${admin_list.admin_id}" onclick="location.href='admin_view.do?board_id=10&admin_id=${admin_list.admin_id}'" value="정보보기" />
									</td>		
								</tr>
								</c:if>
								</c:if>
				...
				<div style="padding-top: 30px;" align="center">
					...
					<td align="right">
						...
						<input class="btn btn-primary" style="font-size: small;" type="button" value="퇴사자 삭제" id="listDelBtn" name="listDelBtn" onclick="detailSubmit(1)" />
						<input class="btn btn-primary" style="font-size: small;" type="button" value="관리자 목록" onclick="adminList();" />
						</c:if>
						</td>
					</tr>
					</table>
				...
				<!-- 페이징 view -->
						<c:if test="${fn:length(admin_list) == 1}">	
					    </c:if>
					    <c:if test="${fn:length(admin_list) >= 2}">	
						<div class="c-pagination" align="center" style="padding-left: 200px; padding-bottom:100px; margin-top:-27px; width: auto;">
							<a href="retire_admin_list.do?startPage=1&visiblePages=10"
							...
							<a href="retire_admin_list.do?startPage=${totalPage}&visiblePages=10"

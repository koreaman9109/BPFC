...
$(document).ready(function(){
    ...
    $("#list").click(function() {
		...
		location.href = "photo_list.do?board_id=3";
	}else{
		return;
		}
			});
...
function detailSubmit(index) {
	    if (index == 1) {
			if(confirm("정말로 수정 하시겠습니까?")) {
				...
				// 폼에 입력한 데이터를 서버로 전송
				document.listForm.action='photoBoardUpdate.do';
	  ...
	  /* 파일추가버튼 */
		function addbt() {
			...
			$('#addtd').append(
					'<tr><td>' + '   <input style="width: 100%;" accept="image/gif, image/jpeg, image/png, image/bmp" type="file" name="uploadfile[' + fileIndex + ']" id="file_up[' + fileIndex + ']"  />' + '</td></tr>');
		};
			...
			<table class="table table-hover">
				...
				<table class="table table-hover">
					<c:choose>
						...
						<c:otherwise>		
							...
							<tr id="flist_${status.count}">
								<c:if test="${file_list.use_yn eq 'Y'}">
								 <td>
									<img style="width: 200px; height: 150px;" src="resources/board/photo/${file_list.file_name2}">
						...
						<tr>
							<td>
							<input  type="file" style="width: 100%;" name="uploadfile[0]" required="required" id="file_up[0]" accept="image/gif, image/jpeg, image/png, image/bmp" />
							</td>
							<td>

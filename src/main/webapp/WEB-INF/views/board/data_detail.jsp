			...
			.ready(
					function() {
						...
						$("#list").click(function() {
							...
							location.href = "data_list.do?board_id=" + $("#board_id").val();
						}); 
						...
						$("#modify")
								.click(
										function() {
											...
											location.href = "dataBoardUpdateForm.do?board_id=" + $("#board_id").val() + "&seq_id=" + $("#seq_id").val();
											}else{
												return;
											}
													});
	 ...
	 function detailSubmit(index) {
		    if (index == 1) {
				if(confirm("정말로 삭제 하시겠습니까?")) {
					document.listForm.action='dataBoardDelete.do';
				...
				<tr>
					...
					<c:forEach var="file_list" items="${file_list}"  varStatus="status">
								...
								<c:if test="${file_list.extention_name eq '.jpg'}">
									...
									<img style="width: 700px; height: 400px; margin-left: auto; margin-right: auto; display: block;" src="resources/board/data/${file_list.file_name2}">
								</c:if>
